...
The code being tested is available in the git commit hash: `r system("git log --pretty=format:'%H' -n 1", intern=TRUE)`
...
for(i in 1:n)
{
  cat("### Model Run",i,"\n\n")
  parm <- inits.sim[i,]
  cat("$$y_{ij} = ",parm[1], parm[3]," s_i ", parm[5]," s_i t_{ij}", parm[2]," t_{ij} ", parm[4]," c_i + b_{0i} + b_{1i} t_{ij} + \\epsilon_{ij}$$\n", sep='')
...
where $t_{ij}$ represents the $j$th timepoint for the $i$th individual, $g_i$ is the presence or absence of a SNIP or expensive binomial information for the $i$th individual, $c_i$ is the confounder for the $i$th individual. Membership in the $g_i$ group occurs at a `r paste0(prev.grp, collapse=", ")` probability.

This was used to create between 4 to 6 observations at equally spaced time points for each individual.

During each of the `r N` runs approximately `r paste0(rowSums(NsPerStratumUniv.sim), collapse=", ")` individuals were sampled using a Bernoulli strategy for stratum. Cutoffs quantiles for sampling were as follows: `r paste0("[",paste(100*(1-p.central)/2, 100-100*(1-p.central)/2, sep=","),"]", collapse=", ")`
