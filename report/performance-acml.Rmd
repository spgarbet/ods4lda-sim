...
results$Sampling <- factor(c("Random", "Intercept", "Slope", "Mix1", "Mix2"),
                           levels=c("Random", "Intercept", "Slope", "Mix1", "Mix2"))
```
...
The code being tested is available in the git commit hash `d14bac64a8f134ad1d50092f226a8cfab4b01320`.

### Method
2000 individuals were simulated using 8 mixed effect models,
...
$$y_{ij} = 75 - 1 t_{ij} - \frac{1}{2} g_i  -2 c_i -\frac{1}{2} t_{ij} g_i  + b_{0i} + b_{1i} t_{ij} + \epsilon_{ij}$$
...
The 8, $k=[1,8]$, different models are defined by the parameters:

$$ S_k = {81, 81, 81, 81, 81, 81, 81, 81}$$
...
The fitting is done by 15 different methods. Each of the combinations of 4 sampling methods: random, intercept, slope and mixture1 and mixture2 crossed with 4 fitting methods: ACML, weighted likelihood, direct multiple imputation (50 replicates) and Two Phase exluding one duplicate method for random sampling (wl).
...
eff <- function(m)
{
  tbl <- purrr::reduce(lapply(1:8, function(i) eff.inner(m, i)), rbind)
    
  attr(tbl, "caption") <- paste0("Efficiency ", m)
  attr(tbl, "style")   <- "nejm"
...
```{r eff_mi, results='asis', echo=FALSE}
html5(eff("MI"))
```

#### Two Phase

```{r eff_twophase, results='asis', echo=FALSE}
html5(eff("TwoPhase"))
```
