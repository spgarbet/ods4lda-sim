...
scenarios <- rbind(c(75, -1, -1, -0, -.3, log(9),log(1),0,log(3.5)),
...
The code being tested is available in the git commit hash `9c67df2f0b62ab32b1fd76f3d75e81bda1cca834`.

### Method
1200 individuals were simulated using 4 mixed effect models,

$$y_{ij} = 75 - 1 t_{ij} - 1 g_i - R_k c_i - 0.3 t_{ij} g_i  + b_{0i} + b_{1i} t_{ij} + \epsilon_{ij}$$
...
This was used to create 5 observations at equally spaced time points for each individual.

During each of the 2000 runs approximately 250 individuals were sampled using a Bernoulli strategy to target 200 in the lower and upper 10% of targetted stratum (intercept, slope or bivariate) with 50 individuals in the middle 80% of stratum.
...
The fitting is done by 15 different methods. Each of the combinations of 4 sampling methods: random, intercept, slope and bivariate crossed with 4 fitting methods: ACML, weighted likelihood, direct multiple imputation (200 replicates) and indirect multiple imputation exluding one duplicate method for random sampling (wl).
...
cell_func <- function(table, row, column, fun=NULL, ...)
{
  ...
  for(m in major)
  {
    ...
    for(n in minor)
    {
      table <- row_header(table, first, n, sub=FALSE)
      first <- ""
